...
int main(int argc, char **argv) {
    ...
    int outXOR3[s6_size*(qu+mi-1)];
     ...
     for (i = 0; i < s5_size*(qu+mi-1); i++)
        inpXOR2[i+s5_size*(qu+mi-1)] = one;
     for (i = 0; i < s5_size*(qu+mi-1); i++)
        inpXOR2[i] = zero;

     for (i = 0; i < s6_size*(qu+mi-1); i++)
        inpXOR3[i+s6_size*(qu+mi-1)] = zero;
     for (i = 0; i < s6_size*(qu+mi-1); i++)
        inpXOR3[i] = one;

     XORCircuit(&garbledCircuit, &garblingContext, s5_size*(qu+mi-1)*2, inpXOR2, outXOR2);
     XORCircuit(&garbledCircuit, &garblingContext, s6_size*(qu+mi-1)*2, inpXOR3, outXOR3);

     for (i = 0; i < (qu+mi-1); i++)
        for (j = 0; j < s5_size; j++)
                inpAND1[i*s5_size+j] = one;
     for (i = 0; i < s5_size*(qu+mi-1); i++)
        inpAND1[i+s5_size*(qu+mi-1)] = outXOR2[i];

     ANDNewCircuit(&garbledCircuit, &garblingContext, s5_size*(qu+mi-1)*2, inpAND1, outAND1);

     for (i = 0; i < s5_size*(qu+mi-1); i++)
        inpXOR2[i] = one;
     for (i = 0; i < s5_size*(qu+mi-1); i++)
        inpXOR2[i+s5_size*(qu+mi-1)] = zero;
