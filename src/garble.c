...
long garbleCircuit(GarbledCircuit *garbledCircuit, InputLabels inputLabels, OutputMap outputMap) {
	...
	for(i=0; i< garbledCircuit->q;i++) {
		...
		if (garbledGate->type != ANDNEWGATE & garbledGate->type != NOTGATE )  {
			keys[0] = xorBlocks(A0, B0);
			keys[0] = xorBlocks(keys[0], tweak);
			keys[1] = xorBlocks(A0,B1);
			keys[1] = xorBlocks(keys[1], tweak);
			...
			newToken2 = xorBlocks(garblingContext.R, newToken);
		}
		else  {
			idx0++;
			idx1++;
			...
			block plainTp[2];
			block Ap, ApR;
        		Ap = Hash(A0, idx0, &(garblingContext.dkCipherContext.K));
        		ApR = Hash(A1, idx0, &(garblingContext.dkCipherContext.K));
		
			if (lsb0 == 0)
        		{
                		inp[0] = Ap;
                		plainT[0] = xorBlocks(ApR, inp[0]);
				plainTp[0] = Ap;
        		}
        		else
        		{
                		inp[0] = xorBlocks(ApR, garblingContext.R);
                		plainT[0] = xorBlocks(Ap, inp[0]);
				plainTp[0] = xorBlocks(Ap, plainT[0]);
        		}
		
			block Bp, BpR;
        		Bp = Hash(B0, idx1, &(garblingContext.dkCipherContext.K));
        		BpR = Hash(B1, idx1, &(garblingContext.dkCipherContext.K));

			inp[1] = Bp;
        		block temp1 = xorBlocks(BpR, inp[1]);
			block Aprime = DOUBLE(garbledCircuit->wires[input0].label);
        		plainT[1] = xorBlocks(Aprime, temp1);
			...
			if (lsb1 == 0)
        		...
        		{
				block temp2 = xorBlocks(Aprime, plainT[1]);
				plainTp[1] = xorBlocks(Bp, temp2);
        		}
		
			garbledTable[tableIndex].table[0] = plainT[0];
			garbledTable[tableIndex].table[1] = plainT[1];

			garbledCircuit->wires[output].label0 = xorBlocks(plainTp[0], plainTp[1]);
			garbledCircuit->wires[output].label1 =  xorBlocks(garblingContext.R, garbledCircuit->wires[output].label0);
		...
		if (garbledGate->type == NOTGATE) {
			garbledCircuit->wires[garbledCircuit->outputs[i]].label0 = garbledCircuit->wires[input1].label1;
			garbledCircuit->wires[garbledCircuit->outputs[i]].label1 = garbledCircuit->wires[input1].label0;
			garbledTable[tableIndex].table[0] = garbledCircuit->wires[input1].label1;
			garbledTable[tableIndex].table[1] = garbledCircuit->wires[input1].label0;
			tableIndex++;
			continue;
		}
...
block Hash(block a, int idx, AES_KEY *k)
{
	a = _mm_slli_epi16(a, 1);

	block temp = _mm_cvtsi32_si128(idx);
	
	block temp1 = xorBlocks(temp, a);
	block temp2[1];
	temp2[0] = temp1;

	AES_ecb_encrypt_blks(temp2, 1, k);

	block result;
	result = xorBlocks(temp2[0], temp1);
