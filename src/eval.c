...
int evaluate(GarbledCircuit *garbledCircuit, ExtractedLabels extractedLabels,
	...
	for (i = 0; i < garbledCircuit->q; i++) {
		...
		if (garbledGate->type == ANDNEWGATE)  {
			block first, second;			
			A = DOUBLE(garbledCircuit->wires[garbledGate->input0].label);
			B = DOUBLE(DOUBLE(garbledCircuit->wires[garbledGate->input1].label));
			idxp0++;
			idxp1++;
			block HA, HB;
                        HA = Hash(A, idxp0, &(garblingContext.dkCipherContext.K));
                        HB = Hash(B, idxp1, &(garblingContext.dkCipherContext.K));
			plainText = &garbledCircuit->wires[garbledGate->output].label;
			...
			if (a == 0)
				...
				first = xorBlocks(HA, garbledTable[tableIndex].table[0]);	

			if (b == 0)
				second = HB;
			else
			{
				second = xorBlocks(HB, garbledTable[tableIndex].table[1]);	
				second = xorBlocks(second, garbledCircuit->wires[garbledGate->input0].label);
			}

			garbledCircuit->wires[garbledGate->output].label =
			xorBlocks(first, second);
			tableIndex++;
		}
		else if (garbledGate->type == NOTGATE)  {
			b = getLSB(garbledCircuit->wires[garbledGate->input1].label);
			if ( b == 0)
				garbledCircuit->wires[garbledGate->output].label = garbledTable[tableIndex].table[0];
			else
				garbledCircuit->wires[garbledGate->output].label = garbledTable[tableIndex].table[1];
