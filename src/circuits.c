...
int AND1Circuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	for (i = 0; i < n/2; i++)
	{
		...
		ANDNewGate(garbledCircuit, garblingContext, inputs[i], inputs[j],
			internalWire);
...
int ANDCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	ANDNewGate(garbledCircuit, garblingContext, inputs[0], inputs[1],
			oldInternalWire);

	for (i = 2; i < n - 1; i++) {
		newInternalWire = getNextWire(garblingContext);
		ANDNewGate(garbledCircuit, garblingContext, inputs[i], oldInternalWire,
				newInternalWire);
		oldInternalWire = newInternalWire;
	}
	outputs[0] = getNextWire(garblingContext);
	return ANDNewGate(garbledCircuit, garblingContext, inputs[n - 1],
			oldInternalWire, outputs[0]);
}

int ANDNewCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
		int n, int* inputs, int* outputs) {
	int i;
	int oldInternalWire = getNextWire(garblingContext);
	int newInternalWire;
	...
	for (i = 0; i < n - 1; i++) {
		...
		if (i % 3 == 2)
			ORGate(garbledCircuit, garblingContext, inputs[i + 1],
					oldInternalWire, newInternalWire);
		if (i % 3 == 1)
			ANDNewGate(garbledCircuit, garblingContext, inputs[i + 1],
					oldInternalWire, newInternalWire);
		if (i % 3 == 0)
...
int RANDCircuit(GarbledCircuit *garbledCircuit,
	...
	ANDNewGate(garbledCircuit, garblingContext, 0, 1, oldInternalWire);

	for (i = 2; i < q + qf - 1; i++) {
		newInternalWire = getNextWire(garblingContext);
		if (i < q)
			ANDNewGate(garbledCircuit, garblingContext, i % n, oldInternalWire,
					newInternalWire);
		else
...
int INCCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	XORGate(garbledCircuit, garblingContext, inputs[0], 1, outputs[0]);
	int carry = inputs[0];
	int newCarry;
	for (i = 1; i < n; i++) {
		XORGate(garbledCircuit, garblingContext, inputs[i], carry, outputs[i]);
		newCarry = getNextWire(garblingContext);
		ANDNewGate(garbledCircuit, garblingContext, inputs[i], carry, newCarry);
		carry = newCarry;
	}
	return 0;
}
...
int MULCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	for (i = 0; i < n; i++) {
		...
		for (j = i; j < i + n; j++) {
			...
			ANDNewGate(garbledCircuit, garblingContext, A[j - i], B[i],
					tempAnd[i][j]);
		}
		for (j = i + n; j < 2 * n; j++)
			tempAnd[i][j] = fixedZeroWire(garbledCircuit, garblingContext);
...
int MINCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
	...
	XORGate(gc, garblingContext, leqOutput[0], 1, notOutput);
	for (i = 0; i < n / 2; i++) {
		andOneOutput[i] = getNextWire(garblingContext);
		andTwoOutput[i] = getNextWire(garblingContext);
		outputs[i] = getNextWire(garblingContext);
		ANDNewGate(gc, garblingContext, leqOutput[0], inputs[i], andOneOutput[i]);
		ANDNewGate(gc, garblingContext, notOutput, inputs[n / 2 + i],
				andTwoOutput[i]);
...
int MaxCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
               int* inputs, int* outputs) {
    int i;
    int leqOutput[1];
    int andOneOutput[n / 2];
    int andTwoOutput[n / 2];
    int notOutput = getNextWire(garblingContext);
    GRECircuit(gc, garblingContext, n, inputs, leqOutput);
    XORGate(gc, garblingContext, leqOutput[0], 1, notOutput);
    for (i = 0; i < n / 2; i++) {
        andOneOutput[i] = getNextWire(garblingContext);
        andTwoOutput[i] = getNextWire(garblingContext);
...
int LEQCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
	...
	int one = getNextWire(garblingContext);
	one = 1;
	XORGate(gc, garblingContext, tempWires, one, outWire);
	outputs[0] = outWire;
	return 0;
}
...
int GEQCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
	...
	int outWire = getNextWire(garblingContext);
	XORGate(gc, garblingContext, tempWires, 1, outWire);
	outputs[0] = outWire;
	return 0;
}
...
int LESCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
	...
	XORGate(gc, garblingContext, A, one, notA);

	int notB = getNextWire(garblingContext);
	XORGate(gc, garblingContext, B, one, notB);

	int case1 = getNextWire(garblingContext);
	ANDNewGate(gc, garblingContext, A, notB, case1);
	...
	XORGate(gc, garblingContext, notA, one,
			temp1);
	XORGate(gc, garblingContext, B, one,
			temp2);
	ANDNewGate(gc, garblingContext, temp1, temp3, temp3);
	XORGate(gc, garblingContext, temp3, one,
			tmpCase2);

	temp1 = getNextWire(garblingContext);
	temp2 = getNextWire(garblingContext);
	temp3 = getNextWire(garblingContext);

	//ORGate(gc, garblingContext, notA, B, tmpCase2);
	XORGate(gc, garblingContext, tmpCase2, one, case2);

	int tmpCase3 = getNextWire(garblingContext);
	int case3 = getNextWire(garblingContext);
	ANDNewGate(gc, garblingContext, notA, notB, tmpCase3);
	ANDNewGate(gc, garblingContext, tmpCase3, test, case3);

	int notTest = getNextWire(garblingContext);
	int tmpCase4 = getNextWire(garblingContext);
	int case4 = getNextWire(garblingContext);
	ANDNewGate(gc, garblingContext, A, B, tmpCase4);
	XORGate(gc, garblingContext, test, one, notTest);
	ANDNewGate(gc, garblingContext, tmpCase4, notTest, case4);
	...
	XORGate(gc, garblingContext, case1, one,
			temp1);
	XORGate(gc, garblingContext, case2, one,
			temp2);
	ANDNewGate(gc, garblingContext, temp1, temp3, temp3);
	XORGate(gc, garblingContext, temp3, one,
			tempFinal1);
	...
	XORGate(gc, garblingContext, case3, one,
			temp1);
	XORGate(gc, garblingContext, case4, one,
			temp2);
	ANDNewGate(gc, garblingContext, temp1, temp3, temp3);
	XORGate(gc, garblingContext, temp3, one,
			tempFinal2);
	...
	XORGate(gc, garblingContext, tempFinal1, one,
			temp1);
	XORGate(gc, garblingContext, tempFinal2, one,
			temp2);
	ANDNewGate(gc, garblingContext, temp1, temp3, temp3);
	XORGate(gc, garblingContext, temp3, one,
			outputs[0]);
...
int EQUCircuit(GarbledCircuit *gc, GarblingContext *garblingContext, int n,
	...
	for (i = 1; i < n / 2; i++) {
		...
		XORGate(gc, garblingContext, tempWire1, one,
			temp1);
		XORGate(gc, garblingContext, tempWires[i], one,
			temp2);
		ANDNewGate(gc, garblingContext, temp1, temp3, temp3);
		XORGate(gc, garblingContext, temp3, one,
			tempWire2);
	...
	XORGate(gc, garblingContext, tempWire1, one, outWire);
	outputs[0] = outWire;
	return 0;
}
...
int NOTCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	for (i = 0; i < n; i++) {
		outputs[i] = getNextWire(garblingContext);
		XORGate(garbledCircuit, garblingContext, inputs[i], 1, outputs[i]);
	}
	return 0;
}
...
int ADD32Circuit(GarbledCircuit *garbledCircuit,
	...
	ANDNewGate(garbledCircuit, garblingContext, wire1, wire2, wire4);
...
int ADD22Circuit(GarbledCircuit *garbledCircuit,
	...
	ANDNewGate(garbledCircuit, garblingContext, inputs[0], inputs[1], wire2);
	outputs[0] = wire1;
	outputs[1] = wire2;
	return 0;
}
...
int ORCircuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
	...
	int one = getNextWire(garblingContext);
	one = 1;
	XORGate(garbledCircuit, garblingContext, inputs[0], one,
			temp1);
	XORGate(garbledCircuit, garblingContext, inputs[1], one,
			temp2);
	ANDNewGate(garbledCircuit, garblingContext, temp1, temp3, temp3);
	XORGate(garbledCircuit, garblingContext, temp3, one,
			oldInternalWire);
	for (i = 2; i < n - 1; i++) {
		newInternalWire = getNextWire(garblingContext);
		temp1 = getNextWire(garblingContext);
		temp2 = getNextWire(garblingContext);
		temp3 = getNextWire(garblingContext);
		XORGate(garbledCircuit, garblingContext, inputs[i], one,
			temp1);
		XORGate(garbledCircuit, garblingContext, oldInternalWire, one,
			temp2);
		ANDNewGate(garbledCircuit, garblingContext, temp1, temp3, temp3);
		XORGate(garbledCircuit, garblingContext, temp3, one,
			newInternalWire);
	...
	temp2 = getNextWire(garblingContext);
	temp3 = getNextWire(garblingContext);
	XORGate(garbledCircuit, garblingContext, inputs[n-1], one,
			temp1);
	XORGate(garbledCircuit, garblingContext, oldInternalWire, one,
			temp2);
	ANDNewGate(garbledCircuit, garblingContext, temp1, temp3, temp3);
	int res = XORGate(garbledCircuit, garblingContext, temp3, one,
			outputs[0]);
...
int XOR1Circuit(GarbledCircuit *garbledCircuit, GarblingContext *garblingContext,
                int n, int* inputs, int* outputs) {
        int i;
        int internalWire;
        int k = n / 2;
